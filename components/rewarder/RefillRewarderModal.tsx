...
import { ERC20_ABI, Rewarder } from '../../constants';

function RefillRewarderModal({
  isOpen,
  setIsOpen,
  chainId,
  rewarder,
  ...
  const tokenContract = useContract({
    addressOrName: rewarder.rewardToken.id,
                      ...
                      onClick={() => {
                        try {
                          tokenContract.transfer(
                            rewarder.id,
                            parseUnits(amount.toString(), rewarder.rewardToken.decimals)
                          );
                        } catch (error) {
                          console.log(error);
                        }
...
export default RefillRewarderModal;
